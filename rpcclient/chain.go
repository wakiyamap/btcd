...
/////
// FutureCheckpointResult is a future promise to deliver the result of a
// CheckpointAsync RPC invocation (or an applicable error).
type FutureCheckpointResult chan *response

// Receive waits for the response promised by the future and returns the hash of
// the block in the best block chain at the given height.
func (r FutureCheckpointResult) Receive() (*chainhash.Hash, error) {
...
// CheckpointAsync returns an instance of a type that can be used to get the
// result of the RPC at some future time by invoking the Receive function on the
// returned instance.
//
// See Checkpoint for the blocking version and more details.
func (c *Client) CheckpointAsync(blockHeight int64) FutureCheckpointResult {
	cmd := btcjson.NewCheckpointCmd(blockHeight)
	return c.sendCmd(cmd)
}

// Checkpoint returns the hash of the block in the best block chain at the
// given height.
func (c *Client) Checkpoint(blockHeight int64) (*chainhash.Hash, error) {
	return c.CheckpointAsync(blockHeight).Receive()
}
...
// Receive waits for the response promised by the future and returns the hash of
// the block in the best block chain at the given height.
func (r FutureDumpCheckpointResult) Receive() (*chainhash.Hash, error) {
	res, err := receiveFuture(r)
	if err != nil {
		return nil, err
	}
...
func (c *Client) DumpCheckpointAsync(blockHeight int64) FutureDumpCheckpointResult {
	cmd := btcjson.NewDumpCheckpointCmd(blockHeight)
	return c.sendCmd(cmd)
}

// DumpCheckpoint returns the hash of the block in the best block chain at the
// given height.
func (c *Client) DumpCheckpoint(blockHeight int64) (*chainhash.Hash, error) {
	return c.DumpCheckpointAsync(blockHeight).Receive()
}
/////
