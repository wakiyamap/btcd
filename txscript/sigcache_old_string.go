...
type OldSigCacheString struct {
	sync.RWMutex
	validSigs  map[sigInfoString]struct{}
	maxEntries uint
}
...
func NewOldSigCacheString(maxEntries uint) *OldSigCacheString {
	return &OldSigCacheString{validSigs: make(map[sigInfoString]struct{}), maxEntries: maxEntries}
}

// Exists returns true if an existing entry of 'sig' over 'sigHash' for public
// key 'pubKey' is found within the OldSigCache. Otherwise, false is returned.
//
// NOTE: This function is safe for concurrent access. Readers won't be blocked
// unless there exists a writer, adding an entry to the OldSigCache.
func (s *OldSigCacheString) Exists(sigHash wire.ShaHash, sig *btcec.Signature, pubKey *btcec.PublicKey) bool {
	info := sigInfoString{string(sigHash[:]), string(sig.Serialize()),
...
// to the signature cache. In the event that the OldSigCache is 'full', an
// existing entry is randomly chosen to be evicted in order to make space for
// the new entry.
//
// NOTE: This function is safe for concurrent access. Writers will block
// simultaneous readers until function execution has concluded.
func (s *OldSigCacheString) Add(sigHash wire.ShaHash, sig *btcec.Signature, pubKey *btcec.PublicKey) {
	s.Lock()
	defer s.Unlock()
