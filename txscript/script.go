...
func calcWitnessSignatureHash(subScript []parsedOpcode, sigHashes *TxSigHashes,
	...
	if isWitnessPubKeyHash(script) {
		...
		// For p2wsh outputs, the script code is the original script,
		// with all code separators removed, serialized with a var int
		// length prefix.
		var b bytes.Buffer
		wire.WriteVarBytes(&sigHash, 0, rawScript)
		wire.WriteVarBytes(&b, 0, rawScript)
	}
	// TODO(roasbeef): assert output is segwitty within caller
...
// TODO(roasbeef): segwit sig op counting + discounting
