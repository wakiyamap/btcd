...
func NewOldSigCache(maxEntries uint) *OldSigCache {
	return &OldSigCache{validSigs: make(map[sigInfo]struct{}), maxEntries: maxEntries}
}
...
func (s *OldSigCache) Exists(sigHash wire.ShaHash, sig *btcec.Signature, pubKey *btcec.PublicKey) bool {
...
func (s *OldSigCache) Add(sigHash wire.ShaHash, sig *btcec.Signature, pubKey *btcec.PublicKey) {
	s.Lock()
	defer s.Unlock()
