...
func (vm *Engine) Step() (done bool, err error) {
	...
	if vm.scriptOff >= len(vm.scripts[vm.scriptIdx]) {
		...
		} else if (vm.scriptIdx == 1 && vm.witness) || (vm.witness && vm.bip16 && vm.scriptIdx == 3) {
...
func NewEngine(scriptPubKey []byte, tx *wire.MsgTx, txIdx int, flags ScriptFlags,
	...
	if vm.hasFlag(ScriptVerifyWitness) {
		...
		case IsWitnessProgram(scriptPubKey):
			if len(scriptSig) != 0 {
				// TODO(roasbeef): real error
				return nil, fmt.Errorf("sigScript must be empty for witness program")
			}
			vm.witness = true
			vm.witnessVersion, vm.witnessProgram, err = ExtractWitnessProgramInfo(scriptPubKey)
			if err != nil {
				return nil, err
			}
		case IsWitnessProgram(scriptSig):
			// TODO(roasbeef): check is wrong?
			vm.witness = true
			vm.witnessVersion, vm.witnessProgram, err = ExtractWitnessProgramInfo(scriptSig)
			if err != nil {
				return nil, err
			}
		}
