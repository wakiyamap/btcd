...
var (
	...
	// thresholdBucketName is the name of the db bucked used to house cached
	// threshold states.
	...
	// deploymentBucketName is the name of the db bucked used to house the
	...
	// warningBucketName is the name of the db bucked used to house the
...
// and their associated threshold states as well as the assocaited deployment
// parameters.
//
...
// dbFetchTresholdCaches uses an existing database transaction to retrieve
...
func dbFetchTresholdCaches(dbTx database.Tx, caches []thresholdStateCache, bucketPrefix []byte) error {
...
func (b *BlockChain) initThresholdCaches() error {
	...
	err = b.db.View(func(dbTx database.Tx) error {
		...
		err := dbFetchTresholdCaches(dbTx, b.deploymentCaches,
			deploymentBucketName)
		if err != nil {
			return err
		}
		...
		return dbFetchTresholdCaches(dbTx, b.warningCaches,
			warningBucketName)
	})
	if err != nil {
		return err
