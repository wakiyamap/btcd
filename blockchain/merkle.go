...
// is the zeroHash.
func BuildMerkleTreeStore(transactions []*btcutil.Tx, witness bool) []*wire.ShaHash {
...
func ValidateWitnessCommitment(blk *btcutil.Block) error {
	...
	expectedCommitment := sha256.New()
	expectedCommitment.Write(witnessMerkleRoot[:])
	expectedCommitment.Write(coinbaseTx.MsgTx().TxIn[0].Witness[0])
	if !bytes.Equal(expectedCommitment.Sum(nil), witnessCommitment) {
