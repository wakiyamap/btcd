...
func TestCalcSequenceLock(t *testing.T) {
	// Lock up the first 4 blocks of the main chain.
	// (genesis block) -> 1 -> 2 -> 3 -> 4
	...
	// TODO(roasbeef): export mempool height from mempool package
	...
	tests := []struct {
		...
		// isn't disabled.
		{
			tx: btcutil.NewTx(&wire.MsgTx{
				...
				TxIn: []*wire.TxIn{
					...
					&wire.TxIn{
						PreviousOutPoint: utxo,
						Sequence: blockchain.LockTimeToSequence(false, 3) |
							blockchain.RelativeLockTimeDisabled,
					},
					&wire.TxIn{
						PreviousOutPoint: utxo,
						Sequence:         blockchain.LockTimeToSequence(false, 3),
			...
			view: utxoView,
			want: &blockchain.SequenceLock{
				Seconds:     medianTime + (5 << blockchain.RelativeLockSecondsGranularity) - 1,
				BlockHeight: 6,
			},
		},
		...
		// be the time further in the future.
		{
			...
			view: utxoView,
			want: &blockchain.SequenceLock{
				Seconds:     medianTime + (10 << blockchain.RelativeLockSecondsGranularity) - 1,
				BlockHeight: -1,
			},
		},
		// A transaction with two inputs with lock times expressed in
		...
		// chosen.
		{
			...
			view: utxoView,
			want: &blockchain.SequenceLock{
				Seconds:     medianTime + (13 << blockchain.RelativeLockSecondsGranularity) - 1,
				BlockHeight: 12,
			},
		},
