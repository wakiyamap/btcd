...
func TestInvWire(t *testing.T) {
	...
	tests := []struct {
		...
		enc  wire.MessageEncoding // Message encodinf format
	}{
...
func TestInvWireErrors(t *testing.T) {
	...
	tests := []struct {
		...
		{baseInv, baseInvEncoded, pver, wire.BaseEncoding, 0, io.ErrShortWrite, io.EOF},
		// Force error in inventory list.
		{baseInv, baseInvEncoded, pver, wire.BaseEncoding, 1, io.ErrShortWrite, io.EOF},
		// Force error with greater than max inventory vectors.
		{maxInv, maxInvEncoded, pver, wire.BaseEncoding, 3, wireErr, wireErr},
	}
