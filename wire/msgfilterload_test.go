...
func TestFilterLoadMaxFilterSize(t *testing.T) {
	...
	err = msg.BtcDecode(readbuf, wire.ProtocolVersion, wire.BaseEncoding)
	if err == nil {
...
func TestFilterLoadWireErrors(t *testing.T) {
	...
	tests := []struct {
		...
		{
			baseFilterLoad, baseFilterLoadEncoded, pver, wire.BaseEncoding, 0,
			io.ErrShortWrite, io.EOF,
		},
		// Force error in filter.
		{
			baseFilterLoad, baseFilterLoadEncoded, pver, wire.BaseEncoding, 1,
			io.ErrShortWrite, io.EOF,
		},
		// Force error in hash funcs.
		{
			baseFilterLoad, baseFilterLoadEncoded, pver, wire.BaseEncoding, 5,
			io.ErrShortWrite, io.EOF,
		},
		// Force error in tweak.
		{
			baseFilterLoad, baseFilterLoadEncoded, pver, wire.BaseEncoding, 9,
			io.ErrShortWrite, io.EOF,
		},
		// Force error in flags.
		{
			baseFilterLoad, baseFilterLoadEncoded, pver, wire.BaseEncoding, 13,
			io.ErrShortWrite, io.EOF,
		},
		// Force error due to unsupported protocol version.
		{
			baseFilterLoad, baseFilterLoadEncoded, pverNoFilterLoad, wire.BaseEncoding,
			10, wireErr, wireErr,
		},
	}
