...
5. [How do I use bootstrap.dat with monad?](#Importing)
...
### 5. How do I use bootstrap.dat with monad?

monad comes with a separate utility named `addblock` which can be used to import
...
1. Stop monad if it is already running.  This is required since addblock needs to
   access the database used by monad and it will be locked if monad is using it.
